...
public class InventoryManager
{
        ...
        foreach (Inventory inventory in Inventories[type].ToList())
        {
            if (onlyFromStockpiles)
            {
                if (inventory.Tile == null ||
                    inventory.Tile.NestedObject == null ||
                    inventory.Tile.NestedObject.Type != "Stockpile" ||
                    inventory.Tile.NestedObject.HasTypeTag("Stockpile"))
                {
                    continue;
                }
            }
    ...
    {
        return (tile.Inventory == null && tile.NestedObject == null && tile.IsEnterable() == Enterability.Yes) ||
