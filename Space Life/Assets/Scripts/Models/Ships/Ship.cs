...
public class Ship : IPrototypable
{
    ...
    private string[,] nestedObjectTypes;

    private ShipState state;
    private Vector2 position;
    private Vector2 destination;
    private NestedObject berth;
    ...
    public Ship(ShipManager shipManager, Ship proto)
    {
        ...
        for (int x = 0; x < Width; x++)
        {
            for (int y = 0; y < Height; y++)
            {
                ...
                nestedObjectTypes[x, y] = proto.nestedObjectTypes[x, y];
            }
        }
    ...
    /// center of the berth NestedObject when the ship is berthed, so it will usually be
    ...
    /// <summary>
    /// Unwraps the ship into a collection of tiles according to the types defined in the tile and NestedObject arrays.
    ...
    public void UnwrapAtBerth()
    {
        for (int x = 0; x < Width; x++)
        {
            for (int y = 0; y < Height; y++)
            {
                ...
                if (tile.Type.Equals(TileType.Empty) == false || tile.NestedObject != null)
                {
                ...
                if (nestedObjectTypes[x, y] != null)
                {
                    World.Current.NestedObjectManager.PlaceNestedObject(nestedObjectTypes[x, y], tile, false);
                }
            }
        }
    }
    ...
    public void Wrap()
    {
        for (int x = 0; x < Width; x++)
        {
            for (int y = 0; y < Height; y++)
            {
                ...
                // Order reversed in case NestedObject on an empty tile leads to problems
                if (nestedObjectTypes[x, y] != null)
                {
                    tile.NestedObject.Deconstruct();
                }
    ...
    public bool WouldFitInBerth(NestedObject berth)
    {
        for (int x = 0; x < Width; x++)
        {
            for (int y = 0; y < Height; y++)
    ...
    /// </summary>
    private Tile GetTile(World world, NestedObject berth, int x, int y, int z)
    {
    ...
    /// <summary>
    /// Reads NestedObject types that the ship contains while unwrapped and
    /// writes them to the NestedObject type array.
    /// </summary>
    /// <param name="reader">The XML reader.</param>
    private void ReadXmlNestedObjects(XmlReader reader)
    {
        if (reader.ReadToDescendant("NestedObject"))
        {
            do
            {
                int x = int.Parse(reader.GetAttribute("x"));
                int y = int.Parse(reader.GetAttribute("y"));
                nestedObjectTypes[x, y] = reader.GetAttribute("type");
            }
    ...
    /// <summary>
    /// Instantiates the tile and NestedObject type arrays and populates them with null values by default.
    /// </summary>
    private void InstantiateTiles()
    {
        tileTypes = new string[Width, Height];
        nestedObjectTypes = new string[Width, Height];
        for (int x = 0; x < Width; x++)
        {
            for (int y = 0; y < Height; y++)
            {
                tileTypes[x, y] = null;
                nestedObjectTypes[x, y] = null;
            }
        }
    }
}