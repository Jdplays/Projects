...
public class WorldGenerator
{
    ...
    public static string[,] startAreaNestedObjects = new string[0, 0];
    ...
    public static void Generate(World world, int seed)
    {
        ...
        for (int x = 0; x < startAreaWidth; x++)
        {
            for (int y = 0; y < startAreaHeight; y++)
            {
                ...
                if (startAreaNestedObjects[x, y] != null && startAreaNestedObjects[x, y] != string.Empty)
                {
                    world.NestedObjectManager.PlaceNestedObject(startAreaNestedObjects[x, y], tile, true);
                }
            }
        }
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (noiseValue >= asteroidNoiseThreshhold && !IsStartArea(x, y, world))
                    {
                        ...
                        world.NestedObjectManager.PlaceNestedObject("astro_wall", tile, false);

                        if (Random.value <= asteroidResourceChance && tile.NestedObject.Name == "Rock Wall")
                        {
                            if (resources.Length > 0)
                            {
                                ...
                                for (int i = 0; i < resources.Length; i++)
                                {
                                    ...
                                    if (randomweight <= currentweight)
                                    {
                                        ...
                                        NestedObject oreWall = PrototypeManager.NestedObject.Get("astro_wall").Clone();
                                        ...
                                        world.NestedObjectManager.PlaceNestedObject(oreWall, tile, false);
                                        break;
                                    }
                                }
                            }
    ...
    public static void ReadXML()
    {
        ...
        string nestedObjectXmlText = System.IO.File.ReadAllText(filePath);

        XmlTextReader reader = new XmlTextReader(new StringReader(nestedObjectXmlText));
        ...
        while (startArea.Read())
        {
            switch (startArea.Name)
            {
                case "Tiles":
                    ...
                    break; 
                case "NestedObjects":
                    XmlReader objReader = reader.ReadSubtree();

                    startAreaNestedObjects = new string[startAreaWidth, startAreaHeight];

                    while (objReader.Read())
                    {
                        if (objReader.Name == "NestedObject")
                        {
                            int x = int.Parse(objReader.GetAttribute("x"));
                            int y = int.Parse(objReader.GetAttribute("y"));
                            startAreaNestedObjects[x, y] = objReader.GetAttribute("name");
                        }
                    }
