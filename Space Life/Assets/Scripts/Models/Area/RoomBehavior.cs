...
namespace SpaceLife.Rooms
{
    ...
    public class RoomBehavior : IXmlSerializable, ISelectable, IPrototypable, IContextActionProvider
    {
        ...
        private List<NestedObjectRequirement> requiredNestedObject;
        ...
        public RoomBehavior()
        {
            ...
            requiredNestedObject = new List<NestedObjectRequirement>();
            ControlledNestedObject = new Dictionary<string, List<NestedObject>>();
        }
        ...
        public Dictionary<string, List<NestedObject>> ControlledNestedObject { get; private set; }
        ...
        private bool DefaultIsValidRoom(Room room)
        {
            ...
            foreach (NestedObjectRequirement requirement in requiredNestedObject)
            {
                if (allTiles.Count(tile => (tile.NestedObject != null && (tile.NestedObject.Type == requirement.type || tile.NestedObject.HasTypeTag(requirement.typeTag)))) < requirement.count) 
                {
                    return false;
                }
            }
        ...
        private void ReadXmlRequirements(XmlReader readerParent, bool isOptional = false)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    case "NestedObject":
                        // NestedObject must have either Type or TypeTag, try both, check for null later
                        ...
                        requiredNestedObject.Add(new NestedObjectRequirement(type, typeTag, count));
                        break;
                    case "Size":
                        if (!isOptional)
                        {
        ...
        private struct NestedObjectRequirement
        {
            public string type, typeTag;
            public int count;

            public NestedObjectRequirement(string type, string typeTag, int count)
            {
                this.type = type;
