...
public class World : IXmlSerializable
{
    ...
    /// <summary>
    /// Gets the NestedObject manager.
    /// </summary>
    /// <value>The NestedObject manager.</value>
    public NestedObjectManager NestedObjectManager { get; private set; }
    ...
    {        
        NestedObjectManager.OnCameraMoved(cameraBounds);
    }
    ...
    /// <summary>
    /// Reserves the NestedObject's work spot, preventing it from being built on. Will not reserve a workspot inside of the NestedObject.
    /// </summary>
    /// <param name="NestedObject">The NestedObject whose workspot will be reserved.</param>
    ...
    public void ReserveTileAsWorkSpot(NestedObject nestedObject, Tile tile = null)
    {
        if (tile == null)
        {
        ...
        if (nestedObject.Jobs.WorkSpotIsInternal())
        {
            return;
        }
        ...
        GetTileAt(
            ...
            tile.Z)
            .ReservedAsWorkSpotBy.Add(nestedObject);
    }
    ...
    public void WriteXml(XmlWriter writer)
    {
        ...
        writer.WriteStartElement("NestedObjects");
        NestedObjectManager.WriteXml(writer);
        writer.WriteEndElement();
    ...
    private void SetupWorld(int width, int height, int depth)
    {
        ...
        NestedObjectManager = new NestedObjectManager();
        NestedObjectManager.Created += OnNestedObjectCreated;
    ...
    /// Also calls "OnFastUpdate" EventActions on visible NestedObject.
    /// </summary>
    /// <param name="deltaTime">Delta time.</param>
    private void TickEveryFrame(float deltaTime)
    {
        CharacterManager.Update(deltaTime);
        NestedObjectManager.TickEveryFrame(deltaTime);
    ...
    {
        NestedObjectManager.TickFixedFrequency(deltaTime);
    ...
    /// <summary>
    /// Called when a NestedObject is created so that we can regenerate the til graph.
    /// </summary>
    /// <param name="NestedObject">NestedObject.</param>
    private void OnNestedObjectCreated(NestedObject nestedObject)
    {
        if (nestedObject.MovementCost != 1)
        {
            // Since tiles return movement cost as their base cost multiplied
            // by the NestedObject's movement cost, a NestedObject movement cost
            // of exactly 1 doesn't impact our pathfinding system, so we can
            // occasionally avoid invalidating pathfinding graphs.
            // InvalidateTileGraph();    // Reset the pathfinding system
            if (tileGraph != null)
            {   
                tileGraph.RegenerateGraphAtTile(nestedObject.Tile);
            }
        }
    }
    ...
    private void ReadXml_NestedObjects(XmlReader reader)
    {
        if (reader.ReadToDescendant("NestedObject"))
        {
            do
            {
                int x = int.Parse(reader.GetAttribute("X"));
                ...
                NestedObject nestedObject = NestedObjectManager.PlaceNestedObject(reader.GetAttribute("type"), tiles[x, y, z], false, rotation);
                nestedObject.ReadXml(reader);
            }
            while (reader.ReadToNextSibling("NestedObject"));
        }
    }
