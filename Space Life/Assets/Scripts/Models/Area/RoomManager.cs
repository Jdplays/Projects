...
namespace SpaceLife.Rooms
{
    public class RoomManager : IEnumerable<Room>
    {
        ...
        {
            // NestedObject is the piece of NestedObject that may be
            // splitting two existing rooms, or may be the final 
            // enclosing piece to form a new room.
            // Check the NESW neighbours of the NestedObject's tile
            // and do flood fill from them.
            Room oldRoom = sourceTile.Room;

            if (oldRoom != null && splitting)
            {
                // The source tile had a room, so this must be a new piece of NestedObject
                ...
                // If this piece of NestedObject was added to an existing room
        ...
        protected Room ActualFloodFill(Tile tile, Room oldRoom, int sizeOfOldRoom)
        {
            ...
            if (tile.NestedObject != null && tile.NestedObject.RoomEnclosure)
            {
                ...
                if (t.Room != newRoom)
                {
                    ...
                    foreach (Tile t2 in ns)
                    {
                        ...
                        else
                        {
                            ...
                            if (
                                t2.Room != newRoom && (t2.NestedObject == null || t2.NestedObject.RoomEnclosure == false))
                            {
            ...
            if (oldRoom != null)
            {
                ...
                // 1 is subtracted from size of old room to account for tile being filled by NestedObject,
