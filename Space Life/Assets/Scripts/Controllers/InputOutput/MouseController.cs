...
public class MouseController
{
    ...
    private GameObject nestedObjectParent;
    ...
    public MouseController(BuildModeController buildModeController, NestedObjectSpriteController nestedObjectSpriteController, UtilitySpriteController utilitySpriteController, GameObject cursorObject)
    {
        ...
        nestedObjectParent = new GameObject("NestedObject Preview Sprites");
    ...
    private void CalculatePlacingPosition()
    {
        ...
        if (currentMode == MouseMode.BUILD
            && bmc.buildMode == BuildMode.NESTEDOBJECT
            && PrototypeManager.NestedObject.Has(bmc.buildModeType)
            && (PrototypeManager.NestedObject.Get(bmc.buildModeType).Width > 1
            || PrototypeManager.NestedObject.Get(bmc.buildModeType).Height > 1))
        {
            NestedObject nestedObjectToBuild = PrototypeManager.NestedObject.Get(bmc.buildModeType).Clone();
            nestedObjectToBuild.SetRotation(bmc.CurrentPreviewRotation);
            Sprite sprite = nosc.GetSpriteForNestedObject(nestedObjectToBuild.Type);

            // Use the center of the NestedObject.
    ...
    {
        // This handles us left-clicking on NestedObject or characters to set a selection.
    ...
    private void BuildOnDraggedTiles(DragParameters dragParams)
    {
        for (int x = dragParams.StartX; x <= dragParams.EndX; x++)
        {
            ...
            for (int y = begin; y != stop; y += increment)
            {
                ...
                if (bmc.buildMode == BuildMode.NESTEDOBJECT)
                {
                    // Check for NestedObject dragType.
                    NestedObject proto = PrototypeManager.NestedObject.Get(bmc.buildModeType);
    ...
    private void ShowNestedObjectSpriteAtTile(string nestedObjectType, Tile tile)
    {
        ...
        sr.sprite = nosc.GetSpriteForNestedObject(nestedObjectType);

        if (World.Current.NestedObjectManager.IsPlacementValid(nestedObjectType, tile, bmc.CurrentPreviewRotation) &&
            World.Current.NestedObjectManager.IsWorkSpotClear(nestedObjectType, tile) && 
            bmc.DoesBuildJobOverlapExistingBuildJob(tile, nestedObjectType, bmc.CurrentPreviewRotation) == false)
        {
        ...
        go.name = nestedObjectType + "_p_" + tile.X + "_" + tile.Y + "_" + tile.Z;
    ...
    private void ShowUtilitySpriteAtTile(string nestedObjectType, Tile tile)
    {
        GameObject go = new GameObject();
        go.transform.SetParent(nestedObjectParent.transform, true);
        dragPreviewGameObjects.Add(go);
        ...
        SpriteRenderer sr = go.AddComponent<SpriteRenderer>();
        sr.sortingLayerName = "Jobs";
        sr.sprite = usc.GetSpriteForUtility(nestedObjectType);

        // TODO: reimplement this for utilities: bmc.DoesBuildJobOverlapExistingBuildJob(t, nestedObjectType) == false)
        if (World.Current.UtilityManager.IsPlacementValid(nestedObjectType, tile)) 
        {
            sr.color = new Color(0.5f, 1f, 0.5f, 0.25f);
        }
        else
