...
namespace SpaceLife.Pathfinding
{
    public static class Pathfinder
    {
        ...
        /// <summary>
        /// Finds the path to NestedObject.
        /// </summary>
        /// <returns>The path to NestedObject.</returns>
        /// <param name="start">Start tile.</param>
        /// <param name="objectType">Object type of the NestedObject.</param>
        public static List<Tile> FindPathToNestedObject(Tile start, string type)
        {
            if (start == null || type == null)
            {
                return null;
            ...
            Path_AStar resolver = new Path_AStar(World.Current, start, GoalNestedObjectEvaluator(type), DijkstraDistance());
            List<Tile> path = resolver.GetList();

            DebugLogIf(path.Count > 0, "FindPathToNestedObject from: {0}, to: {1}, found {2} [Length: {3}, took: {4}ms]", start, type, path.LastOrDefault(), path.Count, (int)(resolver.Duration * 1000));
            DebugLogIf(path == null, "Failed to find path to NestedObject of type {0}", type);
        ...
        /// <summary>
        /// Evaluates if the goal is a NestedObject of type <paramref name="type"/>.
        /// </summary>
        public static GoalEvaluator GoalNestedObjectEvaluator(string type)
        {
            return current => current.NestedObject != null && current.NestedObject.Type == type;
        }
